...
usage(char *prog_name, const char *more) {
  cerr << more;
  cerr << "usage: " << prog_name << " -v -i input_file -o outfile [-r] [-w outfile_weight]" << endl << endl;
...
main(int argc, char **argv) {
  ...
  if (verbose) {
    cout << "kj: infile: " << infile << "\n";
    cout << "kj:   type: " << type << "\n";
    cout.flush();
  }

  Graph g(infile, type, verbose);

  if (verbose) { cout << "kj:   clean.\n"; }
  cout.flush();

  g.clean(type, verbose);

  if (verbose) { cout << "kj:   if(do_renumber).\n"; }
  cout.flush();


  if (do_renumber)
    g.renumber(type, verbose);

  if (verbose) { cout << "kj:   g.display_binary\n" ;}
  cout.flush();

  g.display_binary(outfile, outfile_w, type, verbose);
