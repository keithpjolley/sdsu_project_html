...
Graph::Graph(char *filename, int type, bool verbose) {
  ifstream finput;
  ...
  if (verbose) {
    cout << "Graph::Graph begin\n";
    cout.flush();
  }

  if (verbose) {
    if(!finput) {
      cout<<"finput: NOT ok"<<endl;
    } else {
      cout<<"finput: ok"<<endl;
    }
  }
  ...
  while (!finput.eof()) {
    ...
    if (verbose) {
      cout << "src: " << src << ", dest: " << dest << ", weight: " << weight << "\n";
      cout.flush();
    } 
    if (finput) {
...
void
Graph::renumber(int type, bool verbose) {
...
void
Graph::clean(int type, bool verbose) {
  for (unsigned int i=0 ; i<links.size() ; i++) {
    map<int, float> m;
...
void
Graph::display(int type, bool verbose) {
  for (unsigned int i=0 ; i<links.size() ; i++) {
    for (unsigned int j=0 ; j<links[i].size() ; j++) {
...
void
Graph::display_binary(char *filename, char *filename_w, int type, bool verbose) {
  ofstream foutput;
