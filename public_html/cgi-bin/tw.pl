    ...
    for my $attr (@attriblist) {
      if (($key eq 'links') and ($attr eq 'source' or $attr eq 'target')) {
        ...
        if ($attr eq 'name') {
          ...
          my $shape = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="14px" height="14px"><circle cx="7" cy="7" r="6" style="fill:' . $foo->{'color'} . ';stroke:purple;stroke-width:1;"></svg>';
          $tmp = $shape . " " . $tmp;
        } else {
          $tmp = fmt($tmp);
        }
